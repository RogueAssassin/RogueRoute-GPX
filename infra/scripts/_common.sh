...
load_env_values() {
  ...
  OSRM_SNAP_MAX_RADIUS_METERS=$(grep -E '^OSRM_SNAP_MAX_RADIUS_METERS=' "$ENV_FILE" | tail -n1 | cut -d= -f2- || true)
...
validate_env_for_mode() {
  ...
  case "$mode" in
    osrm)
      ...
      [[ "$OSRM_SNAP_RADIUS_METERS" =~ ^[0-9]+$ ]] || fail "OSRM_SNAP_RADIUS_METERS must be numeric"
      [[ "$OSRM_SNAP_MAX_RADIUS_METERS" =~ ^[0-9]+$ ]] || fail "OSRM_SNAP_MAX_RADIUS_METERS must be numeric"
      (( OSRM_SNAP_MAX_RADIUS_METERS >= OSRM_SNAP_RADIUS_METERS )) || fail "OSRM_SNAP_MAX_RADIUS_METERS must be at least OSRM_SNAP_RADIUS_METERS"
...
osrm_graph_missing_files() {
  local prefix suffix
  prefix="$(osrm_graph_prefix "$1")"

  # This list follows the files consumed by the current Project OSRM storage
  # layer for an MLD graph. In particular, modern OSRM writes nbg_nodes rather
  # than the obsolete nodes sidecar and needs mldgr after customization.
  ...
  for suffix in "${required_suffixes[@]}"; do
    [[ -f "${prefix}${suffix}" ]] || printf '%s\n' "${prefix}${suffix}"
  done

  # Keep compatibility with graphs generated by older OSRM images while using
  # the current nbg_nodes filename for all new builds.
  if [[ ! -f "${prefix}.osrm.nbg_nodes" && ! -f "${prefix}.osrm.nodes" ]]; then
    printf '%s\n' "${prefix}.osrm.nbg_nodes (or legacy ${prefix}.osrm.nodes)"
  fi
}

osrm_graph_is_ready() {
  [[ -z "$(osrm_graph_missing_files "$1")" ]]
}

report_missing_osrm_graph_files() {
  local graph="$1" missing
  while IFS= read -r missing; do
    [[ -n "$missing" ]] && warn "Missing OSRM runtime file: $missing"
  done < <(osrm_graph_missing_files "$graph")
}

verify_osrm_runtime_graph() {
  load_env_values
  ensure_osrm_mount_available
  if ! osrm_graph_is_ready "$OSRM_GRAPH"; then
    warn "OSRM runtime graph is incomplete for OSRM_GRAPH=$OSRM_GRAPH"
    report_missing_osrm_graph_files "$OSRM_GRAPH"
